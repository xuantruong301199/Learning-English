...
module.exports = class paragraph extends PARAGRAPH_COLL {
    static insert({numerical, unitID, image, english, tiengvietese}){
        return new Promise( async resolve => {
            try {
                let dataInsert ={
                    numerical, image, english, tiengvietese, unit: unitID
                }
                if(!english)
    ...
    static getList () {
        return new Promise(async resolve => {
            try {
                let listPara = await PARAGRAPH_COLL.find();
                if(!listPara)
                return resolve({error: true, message: ' Khong hop le'});
    ...
    static Update({paraID, image, english, tiengvietese, numerical}) {
        return new Promise(async resolve => {
            try {
                ...
                return resolve({ error: true, message: 'khong hop le' });
                let dataUpdate ={
                    image, english, tiengvietese, numerical
                }
                // if(file){
                //     dataUpdate.file = file;
                // }
                // console.log(file);
                let infoAfterUpdate = await PARAGRAPH_COLL.findByIdAndUpdate(paraID, dataUpdate);
